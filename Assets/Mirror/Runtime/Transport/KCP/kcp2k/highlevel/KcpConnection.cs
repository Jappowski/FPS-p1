...
namespace kcp2k
{
    ...
    public abstract class KcpConnection
    {
        ...
        public const int ReliableMaxMessageSize =
            (Kcp.MTU_DEF - Kcp.OVERHEAD - CHANNEL_HEADER_SIZE) * (Kcp.WND_RCV - 1) - 1;
        ...
        private readonly byte[] rawSendBuffer = new byte[Kcp.MTU_DEF];
        ...
        protected void SetupKcp(bool noDelay, uint interval = Kcp.INTERVAL, int fastResend = 0,
        ...
        private void HandleTimeout(uint time)
        {
            ...
            {
                Log.Warning(
                    $"KCP: Connection timed out after not receiving any message for {TIMEOUT}ms. Disconnecting.");
                Disconnect();
            }
        }
        ...
        private bool ReceiveNextReliable(out KcpHeader header, out ArraySegment<byte> message)
        {
            ...
            if (msgSize > 0)
            {
                ...
                {
                    Log.Warning(
                        $"KCP: possible allocation attack for msgSize {msgSize} > buffer {kcpMessageBuffer.Length}. Disconnecting the connection.");
                    Disconnect();
                }
            }
            ...
            if (ReceiveNextReliable(out var header, out var message))
                ...
                switch (header)
                {
                    ...
                    case KcpHeader.Disconnect:
                    {
                        ...
                        Log.Warning(
                            $"KCP: received invalid header {header} while Connected. Disconnecting the connection.");
                        Disconnect();
                        break;
                    }
                }
        ...
        private void TickIncoming_Authenticated(uint time)
        {
            ...
            while (!paused &&
                   ReceiveNextReliable(out var header, out var message))
                // message type FSM. no default so we never miss a case.
                switch (header)
                {
                    case KcpHeader.Handshake:
                    {
                        ...
                        Log.Warning(
                            $"KCP: received invalid header {header} while Authenticated. Disconnecting the connection.");
                        Disconnect();
                        break;
                    }
                    case KcpHeader.Data:
                    {
                        ...
                        {
                            Log.Warning(
                                "KCP: received empty Data message while Authenticated. Disconnecting the connection.");
                            Disconnect();
                        }
        ...
        {
            var time = (uint) refTime.ElapsedMilliseconds;
            ...
            if (msgLength > 0)
            {
                ...
                switch (channel)
                {
                    case (byte) KcpChannel.Reliable:
                    {
                        ...
                        var input = kcp.Input(buffer, 1, msgLength - 1);
                        if (input != 0)
                    ...
                    case (byte) KcpChannel.Unreliable:
                    {
                        ...
                        if (state == KcpState.Authenticated)
                        {
                            ...
                            if (!paused)
                            {
                                var message = new ArraySegment<byte>(buffer, 1, msgLength - 1);
                                OnData?.Invoke(message);
                            }
                        ...
                        else
                        {
                            // should never
                            Log.Warning(
                                $"KCP: received unreliable message in state {state}. Disconnecting the connection.");
                            Disconnect();
                        }
        ...
        private void SendReliable(KcpHeader header, ArraySegment<byte> content)
        {
            ...
            if (1 + content.Count <= kcpSendBuffer.Length) // TODO
            {
                ...
                if (sent < 0) Log.Warning($"Send failed with error={sent} for content with length={content.Count}");
            }
            // otherwise content is larger than MaxMessageSize. let user know!
            else
            {
                Log.Error(
                    $"Failed to send reliable message of size {content.Count} because it's larger than ReliableMaxMessageSize={ReliableMaxMessageSize}");
            }
        }
        ...
        private void SendUnreliable(ArraySegment<byte> message)
        {
            ...
            {
                Log.Error(
                    $"Failed to send unreliable message of size {message.Count} because it's larger than UnreliableMaxMessageSize={UnreliableMaxMessageSize}");
            }
        }

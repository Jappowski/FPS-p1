...
namespace kcp2k
{
    public class Kcp
    {
        ...
        public void Flush()
        {
            var offset = 0; // buffer ptr in original C
            ...
            void FlushBuffer()
            {
                if (offset > 0) output(buffer, offset);
            }
        ...
        public void Update(uint currentTimeMilliSeconds)
        {
            ...
            {
                ts_flush += interval;
                if (Utils.TimeDiff(current, ts_flush) >= 0) ts_flush = current + interval;
                Flush();
            }
        }

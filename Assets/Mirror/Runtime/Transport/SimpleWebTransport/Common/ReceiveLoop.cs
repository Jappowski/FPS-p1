...
namespace Mirror.SimpleWeb
{
    internal static class ReceiveLoop
    {
        ...
        {
            (var conn, var maxMessageSize, var expectMask, var queue, var _) = config;
            ...
            try
            {
                try
                {
                    ...
                    while (client.Connected) ReadOneMessage(config, readBuffer);
        ...
        {
            (var conn, var maxMessageSize, var expectMask, var queue, var bufferPool) = config;
            ...
            if (expectMask) offset = ReadHelper.Read(stream, buffer, offset, Constants.MaskSize);

            var opcode = MessageProcessor.GetOpcode(buffer);
            var payloadLength = MessageProcessor.GetPayloadLength(buffer);
        ...
        {
            (var conn, var _, var expectMask, var queue, var bufferPool) = config;
        ...
        {
            (var conn, var _, var expectMask, var _, var _) = config;

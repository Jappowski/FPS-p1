...
namespace Mirror.SimpleWeb
{
    public static class MessageProcessor
    {
        ...
        {
            var finished = (buffer[0] & 0b1000_0000) != 0; // has full message been sent
            var hasMask =
        ...
        private static void ThrowIfNotFinished(bool finished)
        {
            if (!finished)
                throw new InvalidDataException(
                    "Full message should have been sent, if the full message wasn't sent it wasn't sent from this trasnport");
        }
        ...
        private static void ThrowIfBadOpCode(int opcode)
        {
            // 2 = binary
            // 8 = close
            if (opcode != 2 && opcode != 8) throw new InvalidDataException("Expected opcode to be binary or close");
        }

        /// <exception cref="InvalidDataException"></exception>
        private static void ThrowIfLengthZero(int msglen)
        {
            if (msglen == 0) throw new InvalidDataException("Message length was zero");
        }
        ...
        {
            if (msglen > maxLength) throw new InvalidDataException("Message length is greater than max length");
        }
    }
}
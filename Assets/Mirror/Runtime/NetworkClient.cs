...
namespace Mirror
{
    ...
    public static class NetworkClient
    {
        ...
        [Obsolete(
            "NetworkClient.readyConnection is redundant. Use NetworkClient.connection and use NetworkClient.ready to check if it's ready.")]
        ...
        public static void ConnectHost()
        {
            ...
            var connectionToServer = new LocalConnectionToServer();
            var connectionToClient = new LocalConnectionToClient();
        ...
        [Obsolete(
            "Use RegisterHandler<T> version without NetworkConnection parameter. It always points to NetworkClient.connection anyway.")]
        ...
        private static void RegisterPrefabIdentity(NetworkIdentity prefab)
        {
            ...
            var identities = prefab.GetComponentsInChildren<NetworkIdentity>();
            ...
            {
                var existingPrefab = prefabs[prefab.assetId];
                Debug.LogWarning(
            ...
            if (spawnHandlers.ContainsKey(prefab.assetId) || unspawnHandlers.ContainsKey(prefab.assetId))
                Debug.LogWarning(
                    $"Adding prefab '{prefab.name}' with assetId '{prefab.assetId}' when spawnHandlers with same assetId already exists.");
        ...
        public static void RegisterPrefab(GameObject prefab, Guid newAssetId, SpawnDelegate spawnHandler,
        ...
        //       of people use this.
        // TODO why do we have one with SpawnDelegate and one with SpawnHandlerDelegate?
        public static void RegisterPrefab(GameObject prefab, Guid newAssetId, SpawnHandlerDelegate spawnHandler,
            UnSpawnDelegate unspawnHandler)
        {
            if (newAssetId == Guid.Empty)
            {
        ...
        {
            if (unspawnHandlers.TryGetValue(assetId, out var handler) && handler != null)
            {
                handler(obj);
                return true;
            }
        ...
        [Obsolete(
            "NetworkClient.Ready doesn't need a NetworkConnection parameter anymore. It always uses NetworkClient.connection anyway.")]
        ...
        [Obsolete(
            "NetworkClient.AddPlayer doesn't need a NetworkConnection parameter anymore. It always uses NetworkClient.connection anyway.")]
        ...
        {
            NetworkIdentity.spawned.TryGetValue(netid, out var localObject);
            return localObject;
        }
        ...
        private static NetworkIdentity SpawnPrefab(SpawnMessage message)
        {
            ...
            {
                var obj = Object.Instantiate(prefab, message.position, message.rotation);
            ...
            if (spawnHandlers.TryGetValue(message.assetId, out var handler))
            {
                var obj = handler(message);
                if (obj == null)
                {
        ...
        {
            var identity = GetAndRemoveSceneObject(message.sceneId);
            if (identity == null)
                Debug.LogError(
                    $"Spawn scene object not found for {message.sceneId:X} SpawnableObjects.Count={spawnableObjects.Count}");
        ...
        public static void PrepareToSpawnSceneObjects()
        {
            ...
            var allIdentities = Resources.FindObjectsOfTypeAll<NetworkIdentity>();
        ...
        internal static void OnObjectSpawnFinished(ObjectSpawnFinishedMessage _)
        {
            ...
            foreach (var identity in NetworkIdentity.spawned.Values.OrderBy(uv => uv.netId))
            {
                identity.NotifyAuthority();
                identity.OnStartClient();
                CheckForLocalPlayer(identity);
        ...
        private static void ClearNullFromSpawned()
        {
            ...
            foreach (var id in removeFromSpawned) NetworkIdentity.spawned.Remove(id);
        ...
        private static void OnHostClientObjectHide(ObjectHideMessage message)
        {
            ...
            if (NetworkIdentity.spawned.TryGetValue(message.netId, out var localObject) &&
                localObject != null)
        ...
        private static void OnUpdateVarsMessage(UpdateVarsMessage message)
        {
            ...
            if (NetworkIdentity.spawned.TryGetValue(message.netId, out var localObject) && localObject != null)
        ...
        private static void OnRPCMessage(RpcMessage message)
        {
            ...
            if (NetworkIdentity.spawned.TryGetValue(message.netId, out var identity))
                using (var networkReader = NetworkReaderPool.GetReader(message.payload))
                {
                    identity.HandleRemoteCall(message.componentIndex, message.functionHash, MirrorInvokeType.ClientRpc,
                        networkReader);
                }
        }
        ...
        internal static void OnSpawn(SpawnMessage message)
        {
            ...
            if (FindOrSpawnObject(message, out var identity)) ApplySpawnPayload(identity, message);
        }
        ...
        private static void DestroyObject(uint netId)
        {
            ...
            if (NetworkIdentity.spawned.TryGetValue(netId, out var localObject) && localObject != null)
            {
        ...
        [Obsolete(
            "NetworkClient.Update is now called internally from our custom update loop. No need to call Update manually anymore.")]
            ...
            try
            {
                foreach (var identity in NetworkIdentity.spawned.Values)
                    if (identity != null && identity.gameObject != null)
                    {
                        ...
                        var wasUnspawned = InvokeUnSpawnHandler(identity.assetId, identity.gameObject);
                        if (!wasUnspawned)
                        {

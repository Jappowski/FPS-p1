...
namespace Mirror
{
    ...
    public abstract class NetworkConnection
    {
        ...
        protected static bool ValidatePacketSize(ArraySegment<byte> segment, int channelId)
        {
            ...
            {
                Debug.LogError(
                    $"NetworkConnection.ValidatePacketSize: cannot send packet larger than {Transport.activeTransport.GetMaxPacketSize(channelId)} bytes, was {segment.Count} bytes");
                return false;
            }
        ...
        {
            foreach (var netIdentity in observing) netIdentity.RemoveObserverInternal(this);
            observing.Clear();
        }
        ...
        {
            if (MessagePacking.Unpack(reader, out var msgType))
            {
        ...
        internal void TransportReceive(ArraySegment<byte> buffer, int channelId)
        {
            ...
            // unpack message
            using (var reader = NetworkReaderPool.GetReader(buffer))
            {

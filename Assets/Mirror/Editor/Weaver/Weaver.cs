...
namespace Mirror.Weaver
{
    ...
    internal static class Weaver
    {
        ...
        {
            if (td.IsDerivedFrom<MonoBehaviour>()) MonoBehaviourProcessor.Process(td);
        }
        ...
        private static bool WeaveNetworkBehavior(TypeDefinition td)
        {
            ...
            var modified = false;
            foreach (var behaviour in behaviourClasses) modified |= new NetworkBehaviourProcessor(behaviour).Process();
            return modified;
        }
        ...
        private static bool Weave(string assName, IEnumerable<string> dependencies)
        {
                ...
                new ReaderParameters {ReadWrite = true, ReadSymbols = true, AssemblyResolver = asmResolver}))
            {
                ...
                var modified = ReaderWriterProcessor.Process(CurrentAssembly);
                rwstopwatch.Stop();

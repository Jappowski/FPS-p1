...
namespace Mirror.Weaver
{
    ...
    internal class NetworkBehaviourProcessor
    {
        ...
        private Dictionary<FieldDefinition, FieldDefinition> syncVarNetIds =
        ...
        {
            // only process once
            if (WasProcessed(netBehaviourSubclass)) return false;
        ...
        */
        ...
        public static bool WriteArguments(ILProcessor worker, MethodDefinition method, RemoteCallType callType)
        {
            ...
            var skipFirst = callType == RemoteCallType.TargetRpc
            ...
            foreach (var param in method.Parameters)
            {
                ...
                var writeFunc = Writers.GetWriteFunc(param.ParameterType);
                if (writeFunc == null)
                {
        ...
        private void GenerateConstants()
        {
            ...
            for (var i = 0; i < commands.Count; ++i)
            {
                ...
                GenerateRegisterCommandDelegate(cctorWorker, WeaverTypes.registerCommandDelegateReference,
                    commandInvocationFuncs[i], cmdResult);
            }

            for (var i = 0; i < clientRpcs.Count; ++i)
            {
                var clientRpcResult = clientRpcs[i];
                GenerateRegisterRemoteDelegate(cctorWorker, WeaverTypes.registerRpcDelegateReference,
                    clientRpcInvocationFuncs[i], clientRpcResult.method.Name);
            }

            for (var i = 0; i < targetRpcs.Count; ++i)
                GenerateRegisterRemoteDelegate(cctorWorker, WeaverTypes.registerRpcDelegateReference,
                    targetRpcInvocationFuncs[i], targetRpcs[i].Name);

            foreach (var fd in syncObjects) SyncObjectInitializer.GenerateSyncObjectInitializer(ctorWorker, fd);
        ...
        */
        private void GenerateRegisterRemoteDelegate(ILProcessor worker, MethodReference registerMethod,
        ...
        private void GenerateRegisterCommandDelegate(ILProcessor worker, MethodReference registerMethod,
            MethodDefinition func, CmdResult cmdResult)
        {
            var cmdName = cmdResult.method.Name;
            var requiresAuthority = cmdResult.requiresAuthority;
        ...
        private void GenerateSerialization()
        {
            ...
            serialize.Parameters.Add(new ParameterDefinition("forceAll", ParameterAttributes.None,
                WeaverTypes.Import<bool>()));
            ...
            var baseSerialize = Resolvers.TryResolveMethodInParents(netBehaviourSubclass.BaseType,
                Weaver.CurrentAssembly, SerializeMethodName);
            ...
            foreach (var syncVar in syncVars)
            {
                ...
                var writeFunc = Writers.GetWriteFunc(syncVar.FieldType);
                if (writeFunc != null)
                {
                    worker.Emit(OpCodes.Call, writeFunc);
                }
            ...
            var writeUint64Func = Writers.GetWriteFunc(WeaverTypes.Import<ulong>());
            worker.Emit(OpCodes.Call, writeUint64Func);

            // generate a writer call for any dirty variable in this class

            // start at number of syncvars in parent
            var dirtyBit = Weaver.WeaveLists.GetSyncVarStart(netBehaviourSubclass.BaseType.FullName);
            foreach (var syncVar in syncVars)
            {
        ...
        private void DeserializeField(FieldDefinition syncVar, ILProcessor worker, MethodDefinition deserialize)
        {
            // check for Hook function
            var hookMethod = SyncVarProcessor.GetHookMethod(netBehaviourSubclass, syncVar);
        ...
        private void DeserializeNetworkIdentityField(FieldDefinition syncVar, ILProcessor worker,
            MethodDefinition deserialize, MethodDefinition hookMethod)
        {
            /*
            Generates code like:
        ...
        /// <param name="initialState"></param>
        /// <param name="hookResult"></param>
        private void DeserializeNetworkBehaviourField(FieldDefinition syncVar, ILProcessor worker,
            MethodDefinition deserialize, MethodDefinition hookMethod)
        {
            /*
            Generates code like:
            ...
            */
            ...
            // uint oldNetId = ___qNetId;
            var oldNetId = new VariableDefinition(WeaverTypes.Import<NetworkBehaviour.NetworkBehaviourSyncVar>());
            deserialize.Body.Variables.Add(oldNetId);
            worker.Emit(OpCodes.Ldarg_0);
            worker.Emit(OpCodes.Ldfld, netIdField);
            worker.Emit(OpCodes.Stloc, oldNetId);
            ...
            worker.Emit(OpCodes.Ldarg_1);
            // Read()
            worker.Emit(OpCodes.Call,
                Readers.GetReadFunc(WeaverTypes.Import<NetworkBehaviour.NetworkBehaviourSyncVar>()));
            // netId
            worker.Emit(OpCodes.Stfld, netIdField);
        ...
        /// <param name="initialState"></param>
        /// <param name="hookResult"></param>
        private void DeserializeNormalField(FieldDefinition syncVar, ILProcessor serWorker,
            MethodDefinition deserialize, MethodDefinition hookMethod)
        {
            /*
             Generates code like:
            ...
            var readFunc = Readers.GetReadFunc(syncVar.FieldType);
            if (readFunc == null)
            {
                Weaver.Error($"{syncVar.Name} has unsupported type. Use a supported Mirror type instead", syncVar);
                return;
            ...
            if (hookMethod != null)
            {
                ...
                // Generates: if (!SyncVarEqual);
                var syncVarEqualLabel = serWorker.Create(OpCodes.Nop);
        ...
        private void GenerateDeSerialization()
        {
            ...
            var baseDeserialize = Resolvers.TryResolveMethodInParents(netBehaviourSubclass.BaseType,
                Weaver.CurrentAssembly, DeserializeMethodName);
            ...
            foreach (var syncVar in syncVars) DeserializeField(syncVar, serWorker, serialize);
        ...
        public static bool ReadArguments(MethodDefinition method, ILProcessor worker, RemoteCallType callType)
        {
            ...
            foreach (var param in method.Parameters)
            {
                ...
                var readFunc = Readers.GetReadFunc(param.ParameterType);
                ...
                else if (param.ParameterType.Is<double>()) worker.Emit(OpCodes.Conv_R8);
            }
        ...
        {
            collection.Add(new ParameterDefinition("obj", ParameterAttributes.None,
            ...
            collection.Add(new ParameterDefinition("senderConnection", ParameterAttributes.None,
        ...
        private static bool ValidateFunction(MethodReference md)
        {
        ...
        private static bool ValidateParameters(MethodReference method, RemoteCallType callType)
        {
            for (var i = 0; i < method.Parameters.Count; ++i)
            {
                var param = method.Parameters[i];
                if (!ValidateParameter(method, param, callType, i == 0)) return false;
            }
        ...
        private static bool ValidateParameter(MethodReference method, ParameterDefinition param,
            RemoteCallType callType, bool firstParam)
        {
            var isNetworkConnection = param.ParameterType.Is<NetworkConnection>();
            var isSenderConnection = IsSenderConnection(param, callType);
            ...
            if (!isSenderConnection && isNetworkConnection && !(callType == RemoteCallType.TargetRpc && firstParam))
            {
                if (callType == RemoteCallType.Command)
                    ...
                    Weaver.Error($"{method.Name} has invalid parameter {param}. Cannot pass NetworkConnections",
                        method);
                return false;
            }
        ...
        {
            if (callType != RemoteCallType.Command) return false;
        ...
        private void ProcessClientRpc(HashSet<string> names, MethodDefinition md, CustomAttribute clientRpcAttr)
        {
            if (md.IsAbstract)
            {
            ...
            if (!ValidateRemoteCallAndParameters(md, RemoteCallType.ClientRpc)) return;
            ...
            var rpcCallFunc = RpcProcessor.ProcessRpcCall(netBehaviourSubclass, md, clientRpcAttr);
            // need null check here because ProcessRpcCall returns null if it can't write all the args
            if (rpcCallFunc == null) return;

            var rpcFunc = RpcProcessor.ProcessRpcInvoke(netBehaviourSubclass, md, rpcCallFunc);
            if (rpcFunc != null) clientRpcInvocationFuncs.Add(rpcFunc);
        }

        private void ProcessTargetRpc(HashSet<string> names, MethodDefinition md, CustomAttribute targetRpcAttr)
        {
            if (md.IsAbstract)
            {
            ...
            var rpcCallFunc = TargetRpcProcessor.ProcessTargetRpcCall(netBehaviourSubclass, md, targetRpcAttr);

            var rpcFunc = TargetRpcProcessor.ProcessTargetRpcInvoke(netBehaviourSubclass, md, rpcCallFunc);
            if (rpcFunc != null) targetRpcInvocationFuncs.Add(rpcFunc);
        }

        private void ProcessCommand(HashSet<string> names, MethodDefinition md, CustomAttribute commandAttr)
        {
            if (md.IsAbstract)
            {
            ...
            var cmdCallFunc = CommandProcessor.ProcessCommandCall(netBehaviourSubclass, md, commandAttr);

            var cmdFunc = CommandProcessor.ProcessCommandInvoke(netBehaviourSubclass, md, cmdCallFunc);
        ...
        public static void MarkAsProcessed(TypeDefinition td)
        {
            if (!WasProcessed(td))
            {
                var versionMethod = new MethodDefinition(ProcessedFunctionName, MethodAttributes.Private,
                    WeaverTypes.Import(typeof(void)));

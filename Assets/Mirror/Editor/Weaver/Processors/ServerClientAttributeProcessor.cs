...
namespace Mirror.Weaver
{
    internal static class ServerClientAttributeProcessor
    {
        ...
        {
            var modified = false;
            foreach (var md in td.Methods) modified |= ProcessSiteMethod(md);

            foreach (var nested in td.NestedTypes) modified |= Process(nested);
            return modified;
        }
        ...
        private static bool ProcessSiteMethod(MethodDefinition md)
        {
            ...
            if (md.Body != null && md.Body.Instructions != null) return ProcessMethodAttributes(md);
            return false;
        }

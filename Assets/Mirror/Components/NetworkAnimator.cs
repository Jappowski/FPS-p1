    ...
    public class NetworkAnimator : NetworkBehaviour
    {
        ...
        private void CheckSpeed()
        {
            ...
            if (Mathf.Abs(previousSpeed - newSpeed) > 0.001f)
            {
                ...
                else if (isClient) CmdSetAnimatorSpeed(newSpeed);
            }
        }
            ...
            byte[] parameters)
        {
            ...
            else if (isClient) CmdOnAnimationServerMessage(stateHash, normalizedTime, layerId, weight, parameters);
        }
        ...
        private void SendAnimationParametersMessage(byte[] parameters)
        {
            ...
            else if (isClient) CmdOnAnimationParametersServerMessage(parameters);
        }
        ...
        private void HandleAnimMsg(int stateHash, float normalizedTime, int layerId, float weight, NetworkReader reader)
        {
            ...
            if (stateHash != 0 && animator.enabled) animator.Play(stateHash, layerId, normalizedTime);
        ...
        private void ReadParameters(NetworkReader reader)
        {
            ...
            var dirtyBits = reader.ReadUInt64();
            for (var i = 0; i < parameters.Length; i++)
            {
                if ((dirtyBits & (1ul << i)) == 0)
                    continue;
            ...
            byte[] parameters)
        {
            ...
            using (var networkReader = NetworkReaderPool.GetReader(parameters))
            {
        ...
        private void CmdOnAnimationTriggerServerMessage(int hash)
        {
            ...
            if (!isHostOwner) HandleAnimTriggerMsg(hash);
        ...
        private void CmdOnAnimationResetTriggerServerMessage(int hash)
        {
            ...
            // host should have already the trigger
            var isHostOwner = isClient && hasAuthority;
            if (!isHostOwner) HandleAnimResetTriggerMsg(hash);
